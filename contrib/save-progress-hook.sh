...
#
# Example WD_SAVE_PROGRESS_CMD hook: run a Claude Code skill headlessly with the
# day's interview Q&A as context. This is the starting point to ITERATE on — see
# the "known rough edges" note at the bottom.
#
# winddown invokes this with WD_ANSWERS_FILE exported (the day's Q&A markdown).
set -uo pipefail

: "${WD_PROJECT_DIR:=$HOME/code/redhat/resilience/planning/personal notes}"
: "${WD_SKILL:=/team-awareness:save-progress}"
: "${WD_CLAUDE_BIN:=claude}"
...
: "${WD_ALLOWED_TOOLS:=Skill}"

answers=""
if [ -n "${WD_ANSWERS_FILE:-}" ] && [ -f "$WD_ANSWERS_FILE" ]; then
  answers="$(cat "$WD_ANSWERS_FILE")"
fi
...
cd "$WD_PROJECT_DIR" || { echo "save-progress-hook: bad WD_PROJECT_DIR" >&2; exit 1; }

# shellcheck disable=SC2086  # WD_ALLOWED_TOOLS is an intentional word list
exec "$WD_CLAUDE_BIN" -p "$prompt" \
  --permission-mode dontAsk \
  --permission-prompts none \
  --allowedTools $WD_ALLOWED_TOOLS \
  </dev/null
...
#   names to WD_ALLOWED_TOOLS. Discover names with:
