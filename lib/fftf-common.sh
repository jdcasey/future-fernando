...
#
# fftf-common.sh — shared paths, defaults, and config loading.
...
FFTF_CONFIG_FILE="${XDG_CONFIG_HOME:-$HOME/.config}/fftf/fftf.conf"

# ---- defaults (only fill if unset) ------------------------------------------
: "${FFTF_PROJECTS_DIR:=$HOME/.claude/projects}"

# feature switches
: "${FFTF_BREAK_ENABLED:=1}"
: "${FFTF_CAPTURE_ENABLED:=1}"

# break guard (seconds)
: "${FFTF_BREAK_INTERVAL:=3600}"
: "${FFTF_ACTIVITY_GAP:=600}"
# grace window after `fftf-afk`: the nag goes quiet, but the break is only
# *credited* (clock reset) by a real >=FFTF_ACTIVITY_GAP quiet gap. Keep working
# through the grace and the nag returns — silence requires actually stepping away.
: "${FFTF_BREAK_GRACE:=300}"
...
#     off     = <none>          (no ambient sensing at all; rely on fftf-afk/fftf-pause)
...
: "${FFTF_HUMAN_SCAN_N:=5}"
# pause guard (fftf-pause / fftf-unpause): silence the nag when you CAN'T break, without
# crediting one. The stretch clock keeps running, so you come back overdue.
: "${FFTF_PAUSE_DEFAULT:=1800}"   # default pause length if none given (30m)
: "${FFTF_PAUSE_MAX:=7200}"       # hard cap so a fat-fingered pause can't disable all day (2h)
# tool to read audio-device presence (PipeWire/PulseAudio). Existence check only.
: "${FFTF_PACTL_BIN:=pactl}"
...
: "${FFTF_LOG_ENABLED:=1}"
: "${FFTF_LOG_RETAIN_DAYS:=3}"

# break delivery
: "${FFTF_NOTIFY_STACK:=0}"
: "${FFTF_SOUND_ENABLED:=1}"
# grounding suggestion: attach one randomly-chosen practice to each break nag.
: "${FFTF_GROUNDING_ENABLED:=1}"
: "${FFTF_GROUNDING_FILE:=}"       # empty = the installed data/grounding.txt

# capture
: "${FFTF_CAPTURE_IDLE:=480}"
: "${FFTF_CAPTURE_MAX_AGE:=86400}"
: "${FFTF_CAPTURE_PER_TICK:=2}"
: "${FFTF_CAPTURE_TIMEOUT:=120}"
: "${FFTF_CAPTURE_TAIL_BYTES:=12000}"
: "${FFTF_CAPTURE_DIR:=}"

# LLM backend (shared by every LLM-using support, not just capture)
: "${FFTF_LLM_BACKEND:=claude}"          # claude | ollama
: "${FFTF_LLM_MODEL:=}"                   # optional: override whichever backend's model
: "${FFTF_CLAUDE_BIN:=claude}"
: "${FFTF_CLAUDE_MODEL:=claude-haiku-4-5-20251001}"
: "${FFTF_OLLAMA_URL:=http://localhost:11434}"
: "${FFTF_OLLAMA_MODEL:=llama3.2:3b}"
: "${FFTF_OLLAMA_NUM_CTX:=8192}"
: "${FFTF_OLLAMA_KEEP_ALIVE:=5m}"

# Sentinel embedded in distiller prompts so capture never re-captures its own
# LLM calls (which would otherwise loop). Used by fftf-capture.sh / fftf-tick.
# shellcheck disable=SC2034  # consumed in other sourced files, not this one
FFTF_SENTINEL="FOCUSGUARD_CAPTURE_DISTILL_V1"

# ---- config file overrides defaults -----------------------------------------
# shellcheck source=/dev/null
[ -f "$FFTF_CONFIG_FILE" ] && . "$FFTF_CONFIG_FILE"

# ---- ensure working dirs exist ----------------------------------------------
FFTF_LOG_DIR="$FFTF_STATE_DIR/log"
mkdir -p "$FFTF_STATE_DIR" "$FFTF_STATE_DIR/captured" "$FFTF_STATE_DIR/work" \
         "$FFTF_LOG_DIR" 2>/dev/null || true
...
fftf_last_human_activity() {
  ...
  done < <(find "$FFTF_PROJECTS_DIR" -mindepth 2 -maxdepth 2 -name '*.jsonl' \
             -newermt "$today 00:00:00" -printf '%T@ %p\n' 2>/dev/null \
             | grep -v "/$work_slug/" \
             | sort -rn | head -n "$FFTF_HUMAN_SCAN_N" | cut -d' ' -f2-)
  printf '%s\n' "$best"
}
...
fftf_audio_in_active() {
  command -v "$FFTF_PACTL_BIN" >/dev/null 2>&1 || return 1
  "$FFTF_PACTL_BIN" list short source-outputs 2>/dev/null | grep -q '[^[:space:]]'
}
...
fftf_audio_out_active() {
  command -v "$FFTF_PACTL_BIN" >/dev/null 2>&1 || return 1
  "$FFTF_PACTL_BIN" list short sink-inputs 2>/dev/null | grep -q '[^[:space:]]'
}

# Resolve FFTF_PRESENCE (profile name or comma-list) into a space-separated signal
...
fftf_presence_signals() {
  case "$FFTF_PRESENCE" in
...
fftf_break_away_seconds() {
  ...
  if [ "$FFTF_PRESENCE" = auto ]; then
    if val=$(fftf_idle_seconds); then printf '%s\n' "$val"; return 0; fi
    last=$(fftf_last_human_activity); [ -z "$last" ] && last=0
    printf '%s\n' "$(( now - last ))"; return 0
  fi

  sigs=$(fftf_presence_signals)
  # off / empty: no ambient sensing. Report present (0); breaks are credited only
  # by fftf-afk actually being followed by a real gap — the guard runs on elapsed time.
  [ -z "${sigs// }" ] && { printf '0\n'; return 0; }
  ...
  for s in $sigs; do
    val=""
    case "$s" in
      idle)      val=$(fftf_idle_seconds) || val="" ;;
      audio-in)  fftf_audio_in_active  && val=0 ;;   # present now; absent = no vote
      audio-out) fftf_audio_out_active && val=0 ;;   # present now; absent = no vote
      typed)     last=$(fftf_last_human_activity); [ -z "$last" ] && last=0
                 val=$(( now - last )) ;;
    esac
    [ -n "$val" ] || continue
...
fftf_grounding_suggestion() {
  [ "$FFTF_GROUNDING_ENABLED" = 1 ] || return 1
  local file="$FFTF_GROUNDING_FILE" d line
  if [ -z "$file" ]; then
    d=$(fftf_lib_dir 2>/dev/null) || return 1
...
# Honors $FFTF_LIB, then the installed location, then a repo-relative path.
fftf_lib_dir() {
  if [ -n "${FFTF_LIB:-}" ] && [ -f "$FFTF_LIB/fftf-common.sh" ]; then
    printf '%s\n' "$FFTF_LIB"; return 0
  fi
  local installed="${XDG_DATA_HOME:-$HOME/.local/share}/fftf/lib"
  if [ -f "$installed/fftf-common.sh" ]; then printf '%s\n' "$installed"; return 0; fi
  return 1
}
