...
1. **Persistent break guard.** After ~60 minutes of *continuous* work at the keyboard it
   ...
   absence from the keyboard credits the break and resets the clock on its own. Running `fg-afk`
   ...
   long meeting, an incident), `fg-pause` silences it for a bounded window without crediting
...
2. **When the break nag fires** (~60 min of continuous typing), you have two honest
   ...
   - Run `fg-afk` first if the banner is blaring and you want it to hush while you get up,
     *then* actually leave. `fg-afk` only buys a short grace window; it does **not** count
     as a break, so keeping working through it brings the nag back. (You never *have* to
     run `fg-afk` — walking away is enough.)
   - If you truly can't step away yet, `fg-pause [30m]` hushes it for a bounded window
     **without** crediting a break — the clock keeps running, so you're nagged again when
     the pause ends. It auto-expires (capped at 2h); `fg-unpause` ends it early.
3. **When you get pulled away mid-thread**, do nothing. focusguard notices the session
   ...
   transcript.
4. **Check in any time** with `fg-status` to see where you are in the current
   stretch and how long until the next reminder. `fg-status --log` shows the rolling
...
- **Break guard — desktop presence (stackable signals).** The question is "are you
  *present*?", not "are you typing to Claude?". Long autonomous agent runs and reading a diff
  ...
  `FG_PRESENCE` (e.g. `idle,audio-in,audio-out`); you're "present" if **any** enabled signal
...
Set `FG_PRESENCE` to a profile name or a comma-list of signals. They **stack** — you're
...
| `off` | no ambient sensing at all; rely on `fg-afk` / `fg-pause` |
...
> keep the idle counter pinned low, so a real absence never reaches `FG_ACTIVITY_GAP` and
...
> `fg-status --log` to confirm the pattern (idle staying low across a known absence).
...
- Continuous activity is measured from the enabled presence signals, ignoring absences
  shorter than `FG_ACTIVITY_GAP` (10 min).
...
- A genuine ≥`FG_ACTIVITY_GAP` absence (you actually stepped away, or the computer
  suspended/slept) is the only thing that *credits* the break and resets the clock.
- `fg-afk` hushes the nag now and opens a `FG_BREAK_GRACE` (5 min) grace window so you can
...
- `fg-pause [DURATION]` is for when you **can't** step away (a meeting you can't leave, an
  ...
  auto-expires (default 30 min, capped at `FG_PAUSE_MAX` = 2h) so a forgotten pause can't
  disable the guard all day; `fg-unpause` ends it early.
...
- Each nag also carries one **grounding suggestion** picked at random (breathing, 5-4-3-2-1,
  ...
  `data/grounding.txt` (installed to `<data>/focusguard/data/grounding.txt`) to add your own;
  turn it off with `FG_GROUNDING_ENABLED=0` or point `FG_GROUNDING_FILE` at your own file.

### Rolling detection log

Every tick appends one line to `<state>/log/fg-YYYYMMDD.log` recording what each presence
signal read and the decision taken (`active`, `paused`, `grace`, `credit-break`, `nag:N`).
It's the audit and diagnostic trail — view it with `fg-status --log`. Daily files, retained
`FG_LOG_RETAIN_DAYS` (3) days. Like everything else, it stays on your machine and records
...
## What focusguard observes, and what it deliberately doesn't
...
exfiltration**, and focusguard does neither:
...
- **Auditable.** `fg-status` shows exactly which signals are enabled and what each reads;
  `fg-status --log` shows the full per-tick record. Nothing is hidden.
- **Opt out at any granularity.** Disable individual signals via `FG_PRESENCE`, drop to
  `minimal` (idle only), or `off` for no ambient sensing at all (then use `fg-afk`/`fg-pause`).
  `audio-out` is opt-in precisely because speaker monitoring is the most surveillance-adjacent
  signal. Answer capture is separately disableable with `FG_CAPTURE_ENABLED=0`.
...
notification. Answers accumulate in `<state>/winddown/YYYY-MM-DD.md`, each tagged with a
...
To avoid popups all evening on a day you walk away, winddown hard-stops `WD_HARD_STOP_MIN`
minutes after it starts (default 120). Nothing is saved if it backstops mid-interview.

### winddown/windup configuration

Set these for the timers in `~/.config/winddown/winddown.env` (`KEY=value`, one per line,
no shell quoting; read by both units):

| Var | Default | Meaning |
|-----|---------|---------|
| `WD_START` | `15:15` | Clock time the sequence anchors to |
| `WD_INTERVAL_MIN` | `10` | Minutes between question slots |
| `WD_NAG_SEC` | `120` | Re-show the popup this often until answered |
| `WD_HARD_STOP_MIN` | `120` | Safety backstop after start |
| `WD_SOUND_ENABLED` | `1` | Play chimes |
| `WD_SAVE_PROGRESS_CMD` | *(empty)* | Step-4 hook; run with `WD_ANSWERS_FILE` exported |
| `WD_GOODNIGHT_CMD` | *(empty)* | Step-5 hook; empty = one-shot "good evening" notify |
| `WD_WINDUP_KEY` | `tomorrow` | Which winddown answer windup resurfaces |
| `WD_WINDUP_DIALOG` | `1` | windup shows a zenity popup (plus notification) |

Test the flow immediately (no waiting for 3:15): `WD_INTERVAL_MIN=1 WD_NAG_SEC=20 winddown --now`.

### save-progress hook

The save step is pluggable via `WD_SAVE_PROGRESS_CMD`. `install.sh` installs a starter hook
to `~/.local/bin/wd-save-progress-hook` (source: [`contrib/save-progress-hook.sh`](contrib/save-progress-hook.sh))
that runs a Claude Code skill headlessly with the interview answers as context; point
`WD_SAVE_PROGRESS_CMD` at it. Two gotchas for systemd runs: wrap it in `timeout` (the
interview backstop doesn't cover the save step), and set `WD_CLAUDE_BIN` to the absolute
...
`<state>/winddown/log/save-progress-YYYY-MM-DD.log`, a `wd:save_result` line (exit + log
...
The `WD_GOODNIGHT_CMD` seam (step 5) is where a presence-aware persistent clock-off nag
...
Make sure `~/.local/bin` is on your `PATH` so `fg-afk` works from any terminal.

## Usage

```sh
fg-status                                 # where am I in the current stretch?
fg-status --log                           # rolling detection log (diagnose the guard)
fg-afk                                     # I'm stepping away — hush the nag now
fg-pause 30m                               # I can't break yet — hush without crediting one
fg-unpause                                 # end a pause early
fg-capture --compare --latest             # try capture backends on a real session
systemctl --user start focusguard.service # run one scan right now
journalctl --user -u focusguard.service   # service logs
```
